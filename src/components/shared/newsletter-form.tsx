...
export function NewsletterForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!trimmed) { setError(t.footer.emailRequired); return; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(trimmed)) { setError(t.footer.emailInvalid); return; }

    try {
      const raw = localStorage.getItem(STORAGE_KEYS.newsletter);
      const list: string[] = raw ? JSON.parse(raw) : [];
      if (list.includes(trimmed)) { setError(t.footer.alreadySubscribed); return; }
      list.push(trimmed);
