  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
      <div className="text-center mb-12">
        <h2 className="text-2xl sm:text-3xl font-bold text-foreground mb-3">选择你的AI技能方向</h2>
        <p className="text-muted-foreground max-w-xl mx-auto">六大核心领域，覆盖主流AI应用场景</p>
      </div>
        ...
        {categories.map((card) => (
          <Link key={card.slug} href={`/categories/${card.slug}`}>
            <div className="glass-card glass-card-hover p-5 lg:p-6 h-full cursor-pointer group">
              ...
              <p className="text-xs lg:text-sm text-muted-foreground/70 mb-4 lg:mb-5 line-clamp-2">{descriptions[card.slug] || ""}</p>
