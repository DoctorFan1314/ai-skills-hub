...
function getDb(): Database.Database {
  ...
  try {
    ...
    if (planCount.c > 0) {
      const firstPlan = _db.prepare('SELECT display_name FROM subscription_plans LIMIT 1').get() as { display_name: string } | undefined;
      // If old plans exist (Spark/Flare/Pulse/Nova), replace with new ones
      if (firstPlan && (firstPlan.display_name.includes('Spark') || firstPlan.display_name.includes('Flare') || firstPlan.display_name.includes('Pulse') || firstPlan.display_name.includes('Nova'))) {
