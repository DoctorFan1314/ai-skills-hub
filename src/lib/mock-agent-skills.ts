...
export default async function findSkills(query: string) {
  ...
  "parameters": {
    ...
    "required": ["query"]
      ...
      "SKILL.md": `# find-skills

帮助用户发现并安装代理技能。

## 触发条件

当用户询问如何做某事、寻找技能、或表达扩展功能兴趣时触发。
...
当 Agent 需要获取最新网络信息、搜索特定主题、或验证事实时触发。
...
export default async function agentReach(platform: string, action: string, params: Record<string, unknown>) {
  ...
  "parameters": {
    ...
    "required": ["platform", "action"]
      ...
      "SKILL.md": `# agent-reach

搜索、读取和交互 13+ 主流平台。

## 触发条件

当用户需要从社交媒体获取信息、搜索多平台内容、或读取网页时触发。

## 功能

- Twitter/X、YouTube、Bilibili 等平台搜索
- 小红书、抖音、微信公众号内容获取
- GitHub、Reddit、LinkedIn 交互`,
    },
...
export default async function executeCode(language: string, code: string) {
  ...
  "parameters": {
    ...
    "required": ["language", "code"]
      ...
      "SKILL.md": `# code-executor

在沙盒环境中执行和调试代码。

## 触发条件

当用户需要运行代码、调试错误、或执行测试时触发。
...
当用户需要处理文件、格式转换、或批量文件操作时触发。

## 功能

- 文件读写
- 格式转换（PDF、Markdown、JSON、CSV）
- 图片处理与 OCR
- 数据清洗`,
    },
...
export default async function sendEmail(to: string, subject: string, body: string, options?: {
  ...
  "parameters": {
    ...
    "required": ["to", "subject", "body"]
      ...
      "SKILL.md": `# email-sender

发送和管理电子邮件。

## 触发条件

当用户需要发送邮件通知、批量邮件、或使用邮件模板时触发。
